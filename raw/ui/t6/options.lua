...
require( "T6.Menus.optionsstrattester" ) -- Strat tester
...
if UIExpression.IsInGame() == 0 then
		...
		if not CoD.isSinglePlayer then
			...
			f42_local_strat_tester.brackets:close()
			f42_local_strat_tester.m_skipAnimation = true
		end
	else
		if CoD.isSinglePlayer then
...
LUI.createMenu.OptionsMenu = function ( f43_arg0 )
	...
	f43_local0:registerEventHandler( "strat_tester", CoD.Options.OpenStratTesterSettings )
