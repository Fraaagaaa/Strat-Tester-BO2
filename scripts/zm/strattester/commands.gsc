...
pointscase(who, score)
{
    if(!isdefined(score) || !isdefined(who))
        return;
    if(score == "inf" || score == "infinite")
    {
        who.infpoints = true;
        who thread infpoints();
        return;
    }
    who.infpoints = false;
    who.score = string_to_float(score);
}
...
permacase(player)
{
    if(!isvictismap())
    {
        strattesterprint("This map does not support perman perks", "En este mapa no están implementadas las perman perks");
        return;
    }
...
unlockgensrework()
{
    ...
    gens = getstructarray("s_generator", "targetname");
    while(true)
    {
	...
	foreach (gen in getstructarray( "s_generator", "targetname" ))
	{
		if(gen.script_noteworthy == name)
		{
			if(gen.n_current_progress == 100)
			{
                ...
                gen [[getfunction("maps/mp/zm_tomb_capture_zones", "set_zombie_controlled_area")]]();
			...
			else
			{
                ...
                gen [[getfunction("maps\mp\zm_tomb_capture_zones", "players_capture_zone")]]();
				level setclientfield( gen.script_noteworthy, gen.n_current_progress / 100 );
